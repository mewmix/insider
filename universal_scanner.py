...
# Attempt to import from skim_scanner if available in the same directory
try:
...
def execute_trade(
    ...
    if fee_swap_bps != 30:
        step1["extraData"] = encode(['uint256'], [fee_swap_bps])
    ...
    if fee_borrow_bps != 30:
        step2["extraData"] = encode(['uint256'], [fee_borrow_bps])
    ...
    encoded_nested = encode([f"{step_type}[]"], [ [
    ...
    return execute_monstrosity(w3, [flash_step], private_key, gas_price_gwei, dry_run)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Universal Arbitrage Scanner (2-hop & Triangular)")
    ...
    # Configure triangular scan based on max-hops
    if args.max_hops >= 3:
        args.triangular = True
        if args.auto_execute:
            args.triangular_auto_execute = True
